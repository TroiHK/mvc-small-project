function bigUpload () {
	...
	this.sendFile = function (chunk, numberOfChunks) {
		...
		reader.onloadend = function(evt) {
			...
			xhr.open("POST", '/vendor/BigUpload/inc/bigUpload.php?action=upload&key=' + parent.key, true);
	...
	this.sendFileData = function() {
		...
		xhr = new XMLHttpRequest();
		xhr.open("POST", '/vendor/BigUpload/inc/bigUpload.php?action=finish', true);
		xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		...
		xhr.onreadystatechange = function() {
				...
				if(xhr.readyState == 4) {
					...
					$('#bigUpload #fileUpload').val(response.fileName);
					$('#bigUpload #fileUpload').closest('form').submit();
				}
	...
	this.abortFileUpload = function() {
		...
		xhr = new XMLHttpRequest();
		xhr.open("POST", '/vendor/BigUpload/inc/bigUpload.php?action=abort&key=' + this.key, true);
		xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
