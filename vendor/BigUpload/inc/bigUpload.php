...
class BigUpload
{
	...
	private $tempDirectory = '../../../data/uploads/vol/pdf/tmp/';

	//Final folder for storing uploads
	private $mainDirectory = '../../../data/uploads/vol/pdf/';
	...
	public function abortUpload() {
		var_dump($this->tempDirectory . $this->tmpName);
	...
	public function finishUpload($finalName) {
		if (file_exists($this->mainDirectory . $finalName)) {
            unlink($this->mainDirectory . $finalName);
        }

		if(rename($this->tempDirectory . $this->tmpName, $this->mainDirectory . $finalName)) {
			return json_encode(array('errorStatus' => 0, 'fileName' => $finalName));
		}
		else {
			return json_encode(array(
				'errorStatus' => 1,
